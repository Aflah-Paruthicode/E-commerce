  ...
  try {
    ...
    if (useR) {
      ...
      if (userData) {
        ...
        res.render("registration", {
          message: `you're registration has been failed`,
        });
      }
    }
  } catch (error) {
...
const loadRegisterOtpVerification = async (req, res) => {
  try {
    ...
    res.render("otpVerification", {
      SuccessMessage: `Please check youre mail for OTP`,
      user: user,
    });
  } catch (error) {
    console.log(error.message);
...
const changePassword = async (req, res) => {
  try {
    ...
    if (req.body.password.length < 5 || req.body.password.trim() == "") {
      res.render("forgtP-Npswd", {
        emessage: "Pleese Enter Atleast 5 Characters",
        id: user._id,
      });
    } else if (req.body.password !== req.body.cpassword) {
      res.render("forgtP-Npswd", {
        emessage: "Pleese Enter Equel Passwords",
        id: user._id,
      });
    } else {
      ...
      if (userData.block == 1) {
        ...
        if (passwordMatch) {
          if (userData.is_verified === 0) {
            ...
            sendVerifyMail(user.name, user.email);
            res.render("otpVerification", {
              message: "please check youre email, and verify OTP",
              user: user,
            });
          } else {
...
let loadForgetPassOTPPage = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      res.render("forgtP-sub", {
        message: "Check youre email, then verify OTP",
        id: user._id,
      });
    } else {
  ...
  try {
    ...
    if (userData) {
      ...
      const mailOptions = {
        ...
        html:
          '<p>Hi <span style="font-weight:bold"> ' +
          name +
          '</span> please click here to <a style="font-weight:bold;text-decoration:none" href="http://localhost:5008/change?id=' +
          _id +
